import { Observable, fromEvent, Subscription, timer, BehaviorSubject, distinctUntilChanged } from 'rxjs';
...
const hideOverlay$ = fromEvent(document, 'click').pipe(
    filter((event: Event) => {
        const target = event.target as HTMLElement;
        return infoBox !== null && !infoBox.contains(target) && target !== easterEgg && !easterEgg.contains(target);
    }),
    tap(() => {
        if (infoBox) {
 ...
 */
const scrollTo = (target: string | number, duration: number, offsetY: number = 0) => {
    gsap.to(window, {
        duration,
        scrollTo: {
            y: target,
            offsetY
        }, ease: 'power3', autoKill: true
    });
}
 ...
 */
const smoothScroll$ = fromEvent(document.querySelectorAll<HTMLElement>('[data-smooth-scroll]'), 'click').pipe(
 ...
 */
const parseAttribute = (element: HTMLElement, attr: string, defaultValue: number = 0): number => {
 ...
 */
const getScrollTargets = (element: HTMLElement) => {
